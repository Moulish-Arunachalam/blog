...
function Login(){
    ...
    const sendRequest = async (type) => {
        const response  = await axios.post(`http://localhost:5002/api/user/${type}`, {
            name: inputs.name,
            email: inputs.email,
       ...
       <div className="login-container">
           <form onSubmit={submitHandler}>
               ...
               {isSignup && 
                ...
                <a className="pull-right" onClick={() => setisSignup(!isSignup)}>Go to {isSignup ? "Login" : "Sign Up"} page</a>
            </form>
        </div>
    )
